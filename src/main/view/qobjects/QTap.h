...
{
    Q_OBJECT
    Q_PROPERTY(QString sideName READ sideName WRITE setSideName NOTIFY sideNameChanged)
...
public:
    void setSideName(const QString &a)
    {
        if (a != m_sideName) {
            m_sideName = a;
            emit sideNameChanged();
        }
    }

    QString sideName() const
    {
        return m_sideName;
    }
